...
def get_basic_stats_data(content):
    ...
    for sentence in sentence_list:
        ...
        if len(words) > max_sentence_words_len:
            max_sentence_words_len = len(words)

    paragraph_cnt = content.replace('\n\n', '\n').count('\n') + 1
    words_per_paragraph_avg = float(word_cnt) / float(paragraph_cnt)

    abstract_content = ''.join(content.split('[edit]')[0].split('\n')[:-1])
    abstract_chars_len = len(abstract_content)
    abstract_words_len = len(abstract_content.split())

    sections_content = content.split('\n\n')
    sections_chars_len = [len(item) for item in sections_content]
    sections_words_len = [len(item.split()) for item in sections_content]
    std_dev_sections_chars_len = np.std(sections_chars_len)
    std_dev_sections_words_len = np.std(sections_words_len)

    max_section_chars_len = max(sections_chars_len)
    max_section_words_len = max(sections_words_len)
    min_section_chars_len = min(sections_chars_len)
    min_section_words_len = min(sections_words_len)
    
    text_stats_features = []
    text_stats_features.append(word_cnt)
    text_stats_features.append(char_cnt)
    text_stats_features.append(sentence_cnt)
    text_stats_features.append(max_sentence_words_len)
    text_stats_features.append(words_per_paragraph_avg)
    text_stats_features.append(abstract_chars_len)
    text_stats_features.append(abstract_words_len)
    text_stats_features.append(std_dev_sections_chars_len)
    text_stats_features.append(std_dev_sections_words_len)
    text_stats_features.append(max_section_chars_len)
    text_stats_features.append(max_section_words_len)
    text_stats_features.append(min_section_chars_len)
    text_stats_features.append(min_section_words_len)

    return  word_list, sentence_list, text_stats_features
...
if __name__ == '__main__':
    ...
    text_stats_feature_file_name = './feature_text_stats20200214_sample1000.csv'

    if not os.path.exists(text_stats_feature_file_name):
        with open(text_stats_feature_file_name, 'w', newline='', encoding='utf-8-sig') as t_file:
            csv_writer = csv.writer(t_file)
            csv_writer.writerow(('file_name', 'word_cnt', 'char_cnt', 'sentence_cnt', 'max_sentence_words_len','words_per_paragraph_avg',
                                'abstract_chars_len', 'abstract_words_len', 'std_dev_sections_chars_len', 'std_dev_sections_words_len',
                                 'max_section_chars_len', 'max_section_words_len', 'min_section_chars_len', 'min_section_words_len'))   

    progress_counter = 0
    for file in files[:1000]:
        with open(file_dir + file, 'r', encoding='UTF-8') as f:
            content = f.read()
        word_list, sentence_list, text_stats_features = get_basic_stats_data(content)

        with open(text_stats_feature_file_name, 'a', newline='', encoding='utf-8-sig') as t_file:
            csv_writer = csv.writer(t_file)
            csv_writer.writerow((file, text_stats_features[0], text_stats_features[1], 
                                 text_stats_features[2], text_stats_features[3],
                                 text_stats_features[4], text_stats_features[5], 
                                 text_stats_features[6], text_stats_features[7], 
                                 text_stats_features[8], text_stats_features[9],
                                 text_stats_features[10], text_stats_features[11],
                                 text_stats_features[12]))
        if progress_counter % 1000 == 0:
            print(float(progress_counter) / float(len(files)))
