...
def generate_data_files(file_num_limit, paras_limit):
    sampled_para_encoded_dir = './encoded_para_sep/'  # encoded_para_sep_sampled
    sampled_para_txt_list = './labels_feature_stats_merged_cleaned20200223.csv'  # plain_txt_name_index_sampled.csv

    para_encoded_txts = os.listdir(sampled_para_encoded_dir)
    sampled_label_data = pd.read_csv(sampled_para_txt_list, encoding='utf-8-sig')
    stats_features = np.array(sampled_label_data.iloc[label_index, 10:75])

    # stats_features 标准化
    scaler = preprocessing.StandardScaler() #实例化
    scaler = scaler.fit(stats_features)
    stats_features = scaler.transform(stats_features)
    content_feature = np.zeros((paras_limit, 768))
    ...
    while 1:
        for file_index in range(len(para_encoded_txts)):
            ...
            if len(label) != 0:
                # 截断超过paras_limit个元素的content （para中指section超过paras_limit个的文章）
                for para_index in range(min(paras_limit, content.shape[0])):
                    content_feature[para_index, :] = content[para_index, :]
                stats_feature = stats_features[label_index, :]
                file_number_count += 1

            yield ([content_feature, stats_feature], label)
...
file_num_limit = 3000  # total 45614
paras_limit = 20
...
def Bidirectional_LSTM_sematic_stats(X_train, y_train, learning_rate, dropout_rate, adam_decay):
    ...
    possibility_outputs = Dense(6, activation='softmax', name='label_output', kernel_regularizer=regularizers.l2(0.01))(x)  # softmax  sigmoid
    
    model = Model(inputs=[content_input, stats_input], outputs=[possibility_outputs])  # stats_input
    adam = Adam(lr=learning_rate, decay= adam_decay)
    model.compile(loss='categorical_crossentropy', optimizer=adam, metrics=['accuracy', precision, recall, fmeasure])  # categorical_crossentropy  binary_crossentropy
    # print(model.summary())

    return model


def Bidirectional_RNN_sematic_stats(learning_rate, dropout_rate, adam_decay):
    # content part
    hidden_rnn_dim = 64
    content_input = Input(shape=(paras_limit,768), name='content_bert_input')
    x = Bidirectional(SimpleRNN(hidden_rnn_dim, return_sequences=True), input_shape=(paras_limit,768))(content_input)
    x = Dropout(dropout_rate)(x)
    # 添加attention层
    x = Flatten()(x)
    attention_probs = Dense(2 * hidden_rnn_dim * paras_limit, activation='softmax', name='attention_vec')(x)  # 200*2 * paras_limit
    attention_mul = Multiply()([attention_probs, x])
...
def fitness(learning_rate, dropout_rate,
    ...
    blackbox = model.fit_generator(generate_data_files(file_num_limit, paras_limit),
                                   epochs=epochs,
