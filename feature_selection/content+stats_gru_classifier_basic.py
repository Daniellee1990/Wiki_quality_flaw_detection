...
if __name__ == '__main__':
    ...
    if not os.path.exists('experiment_log.csv'):
        with open('experiment_log.csv', 'w', newline='', encoding='utf-8-sig') as t_file:
            csv_writer = csv.writer(t_file)
            csv_writer.writerow(('flaw_index', 'feature_index', 'precision', 'recall', 'f1_score', 'accuracy', 'TNR', 'training_acc', 'training_loss'))
    ...
    for flaw_index in range(6):
        ...
        with open('experiment_log.csv', 'a', newline='', encoding='utf-8-sig') as t_file:
                csv_writer = csv.writer(t_file)
                csv_writer.writerow((str(flaw_index), 'full', np.mean(kfold_precision), np.mean(kfold_recall), np.mean(kfold_f1_score), np.mean(kfold_acc), np.mean(kfold_TNR), np.mean(kfold_training_acc, axis=0), np.mean(kfold_training_loss, axis=0)))
