...
if __name__ == '__main__':
    ...
    for flaw_index in range(6):
        ...
        evaluation_pure_value  = np.mean(kfold_precision, axis=0).tolist() + np.mean(kfold_recall, axis=0).tolist() + np.mean(kfold_f1_score, axis=0).tolist() + np.mean(kfold_acc, axis=0).tolist() + np.mean(kfold_loss, axis=0).tolist() + [np.mean(kfold_time)]
