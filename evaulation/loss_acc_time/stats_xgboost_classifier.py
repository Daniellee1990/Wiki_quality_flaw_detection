...
if __name__ == '__main__':
    ...
    for flaw_index in range(6):
        ...
        kfold_precision, kfold_recall, kfold_f1_score, kfold_acc, kfold_TNR, kfold_time = [], [], [], [], [], []
        fold_counter = 0
        ...
        evaluation_value = str(no_good_flaw_type) + ' 10 k average evaluation is: ' + ' precision: ' + str(np.mean(kfold_precision)) + ' recall ' + str(np.mean(kfold_recall)) + ' f1_score ' + str(np.mean(kfold_f1_score)) + ' accuracy ' + str(np.mean(kfold_acc)) + ' TNR ' + str(np.mean(kfold_TNR)) + 'time '+ str(np.mean(kfold_time))
